...
def recognize_mask(frame, faceNN, maskNN):
    ...
    # If there is a face, predict the mask and no mask using the maskNN model
    if len(faces) > 0:
        faces = np.array(faces, dtype="float32")
        predictions = maskNN.predict(faces, batch_size=32)
...
# Load the mask recognition model
print("Loading model for mask and no mask recognition...")
maskNN = load_model("MaskRecognition.h5")
...
while True:
    ...
    # Recognize mask and no mask in the frame
    (location, predictions) = recognize_mask(frame, faceNN, maskNN)

    # Loop over the face
    for (face_dimension, predict) in zip(location, predictions):
        (startX, startY, endX, endY) = face_dimension
        (maskON, maskOFF) = predict

        if maskON > maskOFF:
            label = "Mask Recognized."
            color = (0, 255, 0)
        else:
            label = "No Mask Recognized"
            color = (255, 0, 0)
    ...
    cv2.imshow("Mask Recognition", frame)
